...
import type { PluginProbeInfo } from '@vben/types';
...
const probeInfo = ref<PluginProbeInfo | null>(null);
...
async function handlePreviewUpload(options: any) {
  ...
  try {
    await previewPlugin({
      ...
      onSuccess: (data, f) => {
        probeInfo.value = data as PluginProbeInfo;
...
const [FirstForm, firstFormApi] = useVbenForm({
  ...
  handleSubmit: async () => {
    if (!uploadedFile.value || !probeInfo.value) {
      message.warning($t('page.northward.plugin.install.uploadText'));
      return;
    }
    currentTab.value = 1;
  },
...
<template>
  <Modal>
    <Steps :current="currentTab" class="steps">
      <Step :title="$t('page.northward.plugin.install.step1')" />
      <Step :title="$t('page.northward.plugin.install.step2')" />
      <Step :title="$t('page.northward.plugin.install.step3')" />
    </Steps>
    <div class="p-4">
      ...
      <div v-show="currentTab === 1" class="space-y-4">
        <Card>
          <Descriptions :column="2" size="middle" :bordered="true">
            <Descriptions.Item :label="$t('page.northward.plugin.name')">
              <Tag color="volcano">
                {{ probeInfo?.name }}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item :label="$t('page.northward.plugin.pluginType')">
              <Tag color="orange">
                {{ probeInfo?.pluginType }}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item :label="$t('page.northward.plugin.version')">
              <Tag color="cyan">
                {{ probeInfo?.version }}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item :label="$t('page.northward.plugin.apiVersion')">
              <Tag color="purple">
                {{ probeInfo?.apiVersion }}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item :label="$t('page.northward.plugin.sdkVersion')">
              <Tag color="pink">
            ...
            <Descriptions.Item :label="$t('page.northward.plugin.size')">
              <Tag color="magenta">
                {{ formatBytes(probeInfo?.size ?? 0) }}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item
              :label="$t('page.northward.plugin.checksum')"
              :span="2"
            >
              {{ probeInfo?.checksum }}
            </Descriptions.Item>
            <Descriptions.Item
              :label="$t('page.northward.plugin.description')"
              :span="2"
            >
          ...
          <div class="mb-4 text-base font-medium">
            {{
              $t('common.installWithName', {
                name: $t('page.northward.plugin.title'),
              })
            }}
          </div>
          <Progress
