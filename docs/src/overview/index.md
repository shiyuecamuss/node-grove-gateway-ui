...
> 记忆法：**southward/northward 是“边界面”，core 是“交通枢纽”。**

### 延伸阅读（建议按需跳转）

- [`南向总览`](../southward/overview.md)：southward 的职责边界、采集策略与容错/性能最佳实践
- [`北向 MQTT v5`](../northward/mqtt-v5.md)：northward 的会话语义、主题/载荷与上下行闭环
- [`核心架构`](./architecture.md#southward)：从系统视角理解 southward/northward 如何被背压与故障隔离串起来
...
- **让北向只关心“业务语义”**：设备、点位、值、时间戳、质量位、事件类型。
- **让规则/转换可复用**：同一条规则可以对 Modbus/S7/OPC UA 的同类点位生效。
- **让可观测性可聚合**：吞吐、延迟、错误率能够按 device/driver/channel/app 维度统计。

> 建议：把“协议地址/寄存器号/NodeId”作为 `meta` 存放（如果必要），不要让它污染北向业务字段。

<a id="collector"></a>

### collector：从“点位清单”到“采集执行”

collector 的职责是把“配置里的采集策略”转成“运行时的调度行为”：

- **轮询采集**：按周期读点位，支持批量读取与拆分策略（避免超出 PDU/帧长/设备能力）。
- **订阅采集**：对支持订阅的协议（如 OPC UA），维护订阅并接收变化事件。
- **优先级与退避**：当设备异常或下游拥塞时，collector 应能自动减速或暂停，并把原因体现在 metrics/log 中。

<a id="northward"></a>

### northward：对接平台的“可靠投递层”（也是最常见的插件形态）

northward 的职责边界建议是：

- **编解码**：把统一模型编码为 json/proto/raw 等格式，并保持版本兼容策略。
- **连接与会话管理**：重连、离线策略、QoS、批发送等。
- **下行处理**：把平台侧命令转换为“控制请求”，再交给 core/southward 执行并回执结果。

> 北向最难的不是“连上”，而是“断了之后语义仍正确”。务必在产品与接口层写清楚投递语义（至少一次/至多一次等）与回执边界。
...
> 最佳实践：无论 stop 还是 reload，都要有“上限时间”
