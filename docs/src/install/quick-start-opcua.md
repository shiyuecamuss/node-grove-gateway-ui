---
title: OPC UA 快速开始（Simulation Server）
---

# OPC UA 快速开始（Simulation Server）
...
### 1.1 准备 `deploy/env/.env`

Compose 文件固定引用 `deploy/env/.env`（`env_file: ../env/.env`）。首次启动前请创建它：

```bash
cat > /path/to/node-grove-gateway/deploy/env/.env << 'EOF'
# ---- Ports (host:container) ----
GATEWAY_HTTP_PORT=8978
GATEWAY_WS_PORT=8979
UI_HTTP_PORT=5678

# ---- Images ----
# You can set these to Docker Hub images later (e.g. your-org/node-grove-gateway:latest).
GATEWAY_IMAGE=node-grove-gateway
GATEWAY_TAG=local
UI_IMAGE=node-grove-ui-web-antd
UI_TAG=local
EOF
```
...
```bash
cd /path/to/node-grove-gateway
docker compose -f deploy/compose/docker-compose.yaml up -d --build
```

#### 方式 B：使用已构建镜像（推荐，CI 推送 Docker Hub 后使用）

```bash
cd /path/to/node-grove-gateway

# 先在 deploy/env/.env 中把 GATEWAY_IMAGE/UI_IMAGE 与 TAG 改成你的镜像仓库地址与版本，
# 然后启动：
docker compose -f deploy/compose/docker-compose.offline.yaml up -d
```

### 1.3 验证服务启动

```bash
docker compose -f deploy/compose/docker-compose.offline.yaml ps
docker compose -f deploy/compose/docker-compose.offline.yaml logs -f --tail=200
```

如果你用的是 `docker-compose.yaml`（本地 build），上面两条命令把 `offline` 文件名替换掉即可。

## 2. 安装 OPC UA Server 模拟器（Prosys）
...
以下字段解释尽量与代码实现一致（默认值来自 `ng-gateway-sdk` 与 `ng-gateway-southward-opcua` 的默认配置）。

### 7.1 基础配置

- **名称**：填写通道名称，例如 `opcua-channel`
- **驱动**：选择 `OPC UA`
- **采集类型**：
  - **被动上报（Report）**：设备/协议栈主动把数据推送给网关，网关被动接收（OPC UA 推荐配合「订阅」模式）
  - **主动采集（Collection）**：网关按周期主动向设备发起读取请求采集数据，需要配合「采集周期」
- **采集周期（period, ms）**：
  - 含义：网关将其映射为后台异步采集任务的周期（单位毫秒）
  - 约束：仅当采集类型为 **主动采集（Collection）** 时必填；本示例使用被动上报可留空
- **上报类型（ReportType）**：
  - **变更上报（Change）**：仅当数据值发生变化时，才会被路由到北向应用
  - **固定上报（Always）**：无论数据是否变化，都会持续路由到北向应用（高频点位请谨慎使用，避免北向与存储压力）

### 7.2 连接策略配置（ConnectionPolicy）

该配置用于控制 **建立连接/读写请求超时** 以及 **断线重连退避策略**。

- **连接超时（connectTimeoutMs, ms）**：建立通道连接的超时时间，默认 `10000`
- **读取超时（readTimeoutMs, ms）**：单次读取请求超时，默认 `10000`（通常用于主动采集的单次任务耗时控制）
- **写入超时（writeTimeoutMs, ms）**：单次写入/控制请求超时，默认 `10000`（例如 `WritePoint` 或命令下发）

#### 重试策略（RetryPolicy, exponential backoff）

用于通道断开/失败后的自动重连（以及部分驱动内部需要重试的 I/O 操作）。

- **最大重试次数（maxAttempts）**：
  - 语义：最大重试次数（`0` 表示不重试；空表示不限制次数）
  - 默认：`3`
- **初始间隔（initialIntervalMs, ms）**：第一次重试前等待时间，默认 `1000`
- **最大间隔（maxIntervalMs, ms）**：重试等待时间的上限，默认 `30000`
...
- **应用名称（applicationName）**：OPC UA Client 的 ApplicationName，例如 `SimulationServer@my-host`
- **应用 URI（applicationUri）**：OPC UA Client 的 ApplicationUri，例如 `urn:my-host.local:OPCUA:SimulationServer`
- **服务器地址（url）**：
...
- **采集模式（readMode）**：
  - **订阅（Subscribe）**：通过 Subscription 实时接收变化（推荐配合「被动上报」）
  - **周期读取（Read）**：按周期发送 Read 请求（通常配合「主动采集 + 采集周期」）
- **会话超时（sessionTimeout, ms）**：OPC UA Client 会话超时时间，默认 `30000`
- **最大失败保活次数（maxFailedKeepAliveCount）**：
  - 语义：连续多少次 KeepAlive 失败后，认为连接不可用并触发重建/重连
  - 默认：`3`
- **保活间隔（keepAliveInterval, ms）**：KeepAlive 心跳间隔，默认 `30000`
...
点击通道表格「操作」列的 **「子设备」** 进入子设备管理界面，点击 **「创建设备」**。

### 9.1 设备配置

- **名称**：例如 `opcua-device`
- **设备类型**：例如 `Sensor`
- **驱动配置**：OPC UA 示例暂无设备级特有配置（保持默认即可）

提交后返回设备列表，确认设备状态正常。

## 10. 创建点位（Point）

在子设备表格「操作」列点击 **「点位管理」**，进入点位界面，点击 **「创建点位」**。
...
1. 在监控页面选择南向通道（例如 `opcua-channel`）
